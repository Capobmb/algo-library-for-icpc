...
/*
    ...
    mo.run(add,del,calc) : add(x), del(x), calc() というラムダ式を渡すと、サイズqのV resが返ってくる。
*/
struct Mo {
    int n, q, width;
    V<int> l, r;
    ...
    auto run(const AL& add_left, const AR& add_right, const DL& del_left, const DR& del_right, const C& calc) -> V<decltype(calc())> {
        q = l.size();
        assert(0 < q);
    ...
    auto run(const A& add, const D& del, const C& calc) -> V<decltype(calc())> {
